...
export default async function submitFileToVercelStorage(formData: FormData) {
    ...
    if (!user || !session || user.role !== "admin") {
        throw new Error("Current user either isn't logged in or doesn't have the required permissions")
    }

    if (!file || !type || !position || !post_id) {
        throw new Error("File content, type, position and post_id are required to submit the file to the server")
    }
